...
- **Featured auctions endpoints:** `POST /auctions/{id}/feature` and `DELETE /auctions/{id}/feature` (admin_ops/admin_super). Configurable max via `auction.featured.max-count` (default 12). Event-sourced (`AuctionFeaturedEvent`/`AuctionUnfeaturedEvent`), auto-unfeatured on auction close. `GET /auctions?featured=true` filters to featured only. Search-service ES index includes `featured`/`featuredAt` fields. Buyer-web homepage fetches featured auctions first, falls back to all active.
...
31. **Catalog-service `listLots()` filters must be combined, not mutually exclusive.** The old `when` block in `LotService.listLots()` checked filters in priority order (sellerId first, then status, etc.), so combining `sellerId + status` silently ignored the status — breaking seller portal quick-status tabs. Fixed by `LotRepository.findByFilters()` which builds a dynamic JPQL WHERE clause combining all non-null filters. Always use `findByFilters()`/`countByFilters()` for lot queries, not individual `findBy*` methods.

32. **Category filtering from buyer-web uses `categorySlug`, not `categoryId`.** The search page sends category slugs (e.g., "office-containers") from the URL. The catalog `LotResource` supports `@QueryParam("categorySlug")` which resolves to a UUID via `LotService.resolveCategorySlug()`. Frontend search composables must use `categorySlug` when the value comes from URL/user input, not `categoryId`.

33. **BidPanel must read reactive state from auction store, not static props.** `BidPanel.vue` receives lot data as props, but props don't update when WebSocket events arrive. Timer (`timerEndTime`), status (`currentAuction.status`), and extension state (`isExtended`) must come from `useAuctionStore()` computed properties with fallback to props. Without this, anti-sniping timer extensions don't reflect in the UI.

34. **AuctionTimer must restart its interval when transitioning from expired back to active.** When anti-sniping extends the auction after the timer has expired, the `isExpired` watcher fires with `wasExpired=true, expired=false`. The `setInterval` must be restarted in this case, otherwise the countdown stays frozen at 00:00.

35. **Admin dashboard `fetchAuctionLots()` must derive from auction detail, not call `/auctions/{id}/lots`.** The auction-engine has a 1:1 auction-to-lot relationship and no `/lots` sub-resource. `fetchAuction()` must complete first, then `fetchAuctionLots()` constructs a single `AuctionLot` entry from the loaded `currentAuction`. Calling `/auctions/{id}/lots` returns 404.

36. **Admin dashboard `fetchLiveBids()` uses `/auctions/{id}/bids`, not `/bids/live`.** The auction-engine has no `/bids/live` endpoint. The response is `ApiResponse<List<BidResponse>>` (a flat array, not paginated `.items`).

37. **Buyer-web search results must pass through `mapAuctionResponse()`.** Without normalization, search items have backend field names (`currentHighBid`, `startingBid`) that don't match `LotCard.vue` expectations (`currentBid`, `startingPrice`). All search result items from both catalog and search-service must be mapped before display.

38. **Admin dashboard `useAuctions.ts` must unwrap `ApiResponse` wrapper.** The backend returns `ApiResponse<PagedResponse<T>>` and `ApiResponse<T>` for auction endpoints. `fetchAuctions()` and `fetchAuction()` must unwrap `raw.data ?? raw` before accessing `.items` or normalizing. Without this, response fields are nested one level deeper and all data appears empty/undefined.

39. **Adding event-sourced domain events requires updates in 5+ coordinated places.** New events (e.g., `AuctionFeaturedEvent`) need: (a) sealed class in `AuctionEvents.kt`, (b) `EVENT_TYPE_REGISTRY` in `AuctionEventEntity.kt`, (c) domain model `apply()` method + command method, (d) read model projection SQL + `updateFromEvent()`, (e) NATS subject mapping in `AuctionLifecycleService`, (f) search-service NATS consumer handler. Missing any one causes silent failures — events store but never project or propagate.
...
70. **`@nuxtjs/tailwindcss` must have `cssPath: false` in buyer-web to preserve PrimeVue styles.** The module auto-injects its own Tailwind CSS *in addition to* `main.css`. This second copy of Preflight (`*, ::before, ::after { border-width: 0; padding: 0; }`) is injected **outside** any CSS layer. CSS rules outside layers always beat rules inside layers, so PrimeVue's `@layer primevue` styles for `border`, `padding`, and `height` get zeroed out on every component (InputText, Button, Select, Checkbox, etc.). Fix: `tailwindcss: { cssPath: false }` in `nuxt.config.ts` — `main.css` already wraps `@tailwind` directives inside proper `@layer` blocks. This does NOT affect seller-portal/admin-dashboard/broker-portal since they use Vite (not Nuxt) and include Tailwind only via their own `main.css`.

71. **Duplicate JSON keys in i18n locale files silently overwrite earlier keys.** JSON spec allows duplicate keys but parsers keep only the last value. The buyer-web locale files had two `"auth"` blocks — the SSO wrong-account translations (added later) overwrote all registration/login translations, causing raw i18n keys (`auth.createAccount`, `auth.business`, etc.) to render on the register page. Fix: always search for existing top-level keys before adding new ones to locale JSON files. Use `grep -c '"keyName":' file.json` to check.

72. **Buyer-web `main.css` must include the same global utility classes as other portals.** The seller-portal, admin-dashboard, and broker-portal all define `.card`, `.label`, `.section-title`, `.page-header`, `.scrollbar-thin` in their `main.css`. Buyer-web was missing all of these (~20 lines vs ~85 lines), causing inconsistent look and feel. When adding new global utility classes to one portal's CSS, check all four portals for alignment.
