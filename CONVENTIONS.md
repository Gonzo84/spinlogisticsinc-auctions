...
### 2.4 Lot ID vs Auction ID (Critical)

Every auction lot has **two different UUIDs** generated by different services:

| Identity | Source | Field Name | Used For |
|----------|--------|-----------|----------|
| Catalog Lot ID | catalog-service | `lotId` (backend), `catalogLotId` (buyer-web) | Lot CRUD, navigation links, images, categories |
| Auction ID | auction-engine | `auctionId`, `aggregateId` (domain events) | Bidding, WebSocket rooms, event sourcing, closing |

**Rules:**
- Frontend navigation to `/lots/{id}` MUST use `catalogLotId`, never `auctionId`
- NATS consumer `extractLotId()` MUST only read the explicit `lotId` field — NEVER fall back to `aggregateId` (which is always the `auctionId` or `settlementId`)
- Payment-service MUST throw/return early if `sellerId` or `auctionId` can't be resolved — never silently substitute `lotId`
- Gateway WebSocket broadcasts MUST include `lotId` alongside `auctionId` for overbid notification links
- See `docs/LOT_AUCTION_ID_AUDIT.md` for the full audit and improvement plan
