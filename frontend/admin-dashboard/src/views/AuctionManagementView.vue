  ...
  <div>
    <div class="page-header">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Create, manage, and monitor auctions.</p>
      </div>
      <router-link to="/auctions/create" class="btn-primary">
    ...
    <div class="card mb-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
        ...
        <div class="flex-1">
          ...
          <input v-model="filters.brand" type="text" class="input" placeholder="Filter by brand..." />
        </div>
        <div class="flex-1">
    ...
    <div v-else-if="error" class="card border-red-200 bg-red-50 text-center">
      ...
      <button class="btn-secondary btn-sm mt-3" @click="fetchAuctions">Retry</button>
    </div>
      ...
      <table class="w-full">
        ...
        <tbody>
          <tr v-if="auctions.length === 0">
            <td colspan="10" class="px-4 py-12 text-center text-sm text-gray-500">No auctions found.</td>
          </tr>
          <tr v-for="auction in auctions" :key="auction.id" class="table-row">
            <td class="table-cell">
            ...
            <td class="table-cell text-gray-600">{{ auction.country }}</td>
            <td class="table-cell">
              <StatusBadge :status="auction.status" />
            </td>
            <td class="table-cell text-right">{{ auction.lotCount }}</td>
            <td class="table-cell text-right">{{ auction.totalBids }}</td>
            <td class="table-cell text-right">{{ auction.buyerPremiumPercent }}%</td>
            <td class="table-cell text-gray-500">{{ formatDate(auction.startDate) }}</td>
            <td class="table-cell text-gray-500">{{ formatDate(auction.endDate) }}</td>
            <td class="table-cell text-right">
              <router-link :to="`/auctions/${auction.id}`" class="btn-secondary btn-sm">
                View
              </router-link>
            </td>
          </tr>
