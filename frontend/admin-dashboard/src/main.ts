...
if (!hasAdminRole) {
  ...
  const logoutUrl = `${keycloak.authServerUrl}/realms/${keycloak.realm}/protocol/openid-connect/logout?client_id=admin-dashboard&post_logout_redirect_uri=${encodeURIComponent(window.location.origin)}`
  ...
  document.body.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:center;height:100vh;font-family:system-ui,-apple-system,sans-serif">
      <div style="text-align:center;max-width:420px;padding:32px;border:1px solid #e5e7eb;border-radius:12px;background:#fff">
        ...
        <p style="margin:0 0 24px;color:#6b7280;font-size:0.875rem">The Admin Dashboard requires administrator privileges. Contact your system administrator if you believe this is an error.</p>
        ...
        ${hasBuyerRole || hasSellerRole || hasBrokerRole ? `
