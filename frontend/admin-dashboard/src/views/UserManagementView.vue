  ...
  <div>
    <div class="page-header">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Manage buyers, sellers, and account statuses.</p>
      </div>
      <p class="text-sm text-gray-500">{{ totalCount }} users total</p>
    </div>
    ...
    <div class="card mb-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
        ...
        <div>
          ...
          <select v-model="filters.accountType" class="select">
        ...
        <div>
          ...
          <select v-model="filters.kycStatus" class="select">
    ...
    <div v-else class="table-container">
      <table class="w-full">
        ...
        <tbody>
          <tr v-if="users.length === 0">
            <td colspan="9" class="px-4 py-12 text-center text-sm text-gray-500">No users found.</td>
          </tr>
          <tr v-for="user in users" :key="user.id" class="table-row">
            <td class="table-cell">
              <router-link :to="`/users/${user.id}`" class="hover:text-primary-600">
                <p class="font-medium text-gray-900">{{ user.firstName }} {{ user.lastName }}</p>
                <p class="text-xs text-gray-500">{{ user.email }}</p>
              </router-link>
            </td>
            <td class="table-cell text-gray-600">{{ user.companyName || '--' }}</td>
            <td class="table-cell">
              <StatusBadge :status="user.accountType" size="sm" />
            </td>
            <td class="table-cell">
              <StatusBadge :status="user.status" />
            </td>
            <td class="table-cell">
              <StatusBadge :status="user.kycStatus" size="sm" />
            </td>
            <td class="table-cell">
              <StatusBadge :status="user.depositStatus" size="sm" />
            </td>
            <td class="table-cell text-gray-500">{{ formatDate(user.registeredAt) }}</td>
            <td class="table-cell text-gray-500">{{ formatDate(user.lastLoginAt) }}</td>
            <td class="table-cell text-right">
              <router-link :to="`/users/${user.id}`" class="btn-secondary btn-sm">
                View
              </router-link>
            </td>
          </tr>
      ...
      <div v-if="totalPages() > 1" class="flex items-center justify-between border-t border-gray-200 px-4 py-3">
        <p class="text-sm text-gray-500">Page {{ filters.page }} of {{ totalPages() }}</p>
